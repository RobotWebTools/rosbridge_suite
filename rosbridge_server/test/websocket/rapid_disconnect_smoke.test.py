...
class TestWebsocketSmoke(unittest.TestCase):
    def sub_many(self, node: Node, duration: float, topic: str, sub_ws_clients, publisher, content) -> Awaitable[None]:
        """
    ...
    async def test_smoke(self, node: Node, make_client):
        ...
        await self.sub_many(node, 0.0001, A_TOPIC, sub_ws_clients, pub_a, A_STRING)
